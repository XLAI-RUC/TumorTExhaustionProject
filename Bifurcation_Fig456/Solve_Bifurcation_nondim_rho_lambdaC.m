function Bifurcation_rho_nondim_lambdaC

epl_0 = 0.01;      % epl_0 = 0.05;  
lambdaC_0 = 0.6;   % 0.7163
...
fun = @(x)root2d(x,epl_0,rho_0,lambdaC_0);

x1 = [0.05,0.05];  % initial value for searching 
RootC1(ind,1:2) = fsolve(fun,x1);  % steady state 1
x2 = [0.2,0.2];    % initial value for searching 
RootC2(ind,1:2) = fsolve(fun,x2);  % steady state 2
x3 = [1,1];       % initial value for searching 
...
dlmwrite([File_folder_name, 'Bifur_epl_0p01_lambdaC_0p6_sol1.dat'],RootC1,' ');
dlmwrite([File_folder_name, 'Bifur_epl_0p01_lambdaC_0p6_sol2.dat'],RootC2,' ');
dlmwrite([File_folder_name, 'Bifur_epl_0p01_lambdaC_0p6_sol3.dat'],RootC3,' ');

end

function Fun = root2d(x,epl_0,rho_0,lambdaC_0)

par = setparameter();

par.epl = epl_0; 
par.rho = rho_0; 
par.lambdaC = lambdaC_0; 
